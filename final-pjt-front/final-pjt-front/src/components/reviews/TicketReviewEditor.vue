<template>
  <div>
        ...
        <!-- 사각형 구분선 -->
        <rect x="8%" y="8%" width="84%" height="82%" fill="none" :stroke="inkColor" stroke-width="4"/>
        <rect x="8%" y="8%" width="84%" height="56%" fill="none" :stroke="inkColor" stroke-width="4"/>
        <rect x="8%" y="8%" width="84%" height="48%" fill="none" :stroke="inkColor" stroke-width="4"/>
        <rect x="8%" y="8%" width="84%" height="24%" fill="none" :stroke="inkColor" stroke-width="4"/>
        <!-- 상자 밖 상단 (로고 / 넘버) -->
        ...
        <!-- 리뷰 날짜 -->
        <template v-if="getTodayDate!=null">
        <text x="50%" y="94%" class="brand-font" :style="'fill:'+inkColor+';'" 
        dominant-baseline="top" text-anchor="middle"> {{ getTodayDate }} </text>
        </template>

        <!-- Text Input -> 에디터용  -->
        <foreignObject x="16%" y="56.5%" width="80%" height="150">
            <div xmlns="http://www.w3.org/1999/xhtml" width="">
                <select name="my-rating" id="my-rating" autofocus required
        ...
        <foreignObject x="10%" y="71%" width="80%" height="150">
            <div xmlns="http://www.w3.org/1999/xhtml" width="">
                <input type="text" placeholder="여기를 클릭해서 리뷰를 써보세요." 
                :style="'fill:'+inkColor+';'" v-model="ticket.content">
            </div>
        </foreignObject>
        ...
        <!-- 작성하기 버튼 -->
        <button @click="submit" :disabled="ticket.content==''?true:false"
        class="btn btn-sm btn-outline-secondary my-3">
            <template v-if="ticket.content==''">
                리뷰 내용을 먼저 작성해주세요 <i class="fas fa-sad-tear"></i>
            </template>
            <template v-else>
                티켓 완성 <i class="fas fa-check-circle"></i>
            </template>
        </button>
...
<script>
import { reviewCreate } from '@/api/index'
import ColorThief from '@/../node_modules/colorthief/dist/color-thief.mjs'

export default {
    name:"TicketReviewEditor",
    data: function(){
        return{
           ticket : {
               content : '',
    ...
    methods:{
        submit : async function(){
            try{
                const movie_id = this.movie.movie_pk // 영화 ID
                
                // axios 요청으로 리뷰티켓을 작성한다
                await reviewCreate(movie_id, this.ticket)                 
                // 티켓박스 페이지로 이동
